...
const useAuthStore = create((set, get: any) => ({
  ...
  login: async (state: any) => {
    ...
    supabaseService.setTable("UserAuthentication");
    ...
    if (response.status) {
      if (
        await encryptor.decrypt(userData.password, response.result.ua_password_hash)
      ) {
        response.result.password = null;
        const clin_id = response.result.clin_id;
        
        supabaseService.setTable("Clinician");

        const clin_response = await supabaseService.selectBy(
          "clin_id",
          clin_id
        );

        localStorage.setItem("csu_user", JSON.stringify(clin_response.result));
