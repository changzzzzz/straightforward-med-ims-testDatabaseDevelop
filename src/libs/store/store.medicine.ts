...
const useMedicineStore = create(
  zukeeper((set, get: any) => ({
    ...
    requestMedication: async (medication: any) => {
      ...
      supabaseService.setTable("Tests");
    ...
    createCabinet: async (cabinet: any) => {
      ...
      // let clincianId = uuidv4();

      const formData = get().form;

      supabaseService.setTable("Clinician");

      //setting clincian name
      _clincian.clin_fname = formData.clinican;
      // _clincian.clin_lname = formData.clinican;
      // _cabinet.createdAt = getCurrentDate();
      // _clincian.clin_id = clincianId;
      console.log("clinician add",_clincian);
      const response = await supabaseService.add(_clincian);
      console.log(_clincian);
      if (!response.status) {
        set((state: any) => ({
          hasChanges: !state.hasChanges,
          notification: {
            type: "error",
            title: "Clinican Creation Failed",
            description:
      ...
      if (response.status) {
        ...
        set((state: any) => ({
          ...
          notification: {
            type: "success",
            title: "Clinician Created",
            description: "Your clinician has been successfully created.",
          },
        }));
      } else {
        set((state: any) => ({
          ...
          notification: {
            type: "error",
            title: "Clinician Creation Failed",
            description:
              "Your clinician could not be created. Please try again later.",
          },
        }));
      }
    },
    ...
    createMedicine: async (medicine: any) => {
      ...
      console.log("medicineData before html",medicineData);
      for (let [key, value] of formData.entries()) {
    ...
    updateMedicine: async (id: any, updatedMedicine: any) => {
      ...
      console.log("update medicineData",medicineData);
      supabaseService.setTable("Patient");
